...
export function EventDetailSheet({ event, calendars, onClose, onSave, onDelete }: EventDetailSheetProps) {
  const [mode, setMode] = useState<"view" | "edit">("view");
  const [draft, setDraft] = useState<CalendarEvent>(event);

  const liveCalendar =
    calendars.find((c) => c.id === draft.calendarId) ?? calendars[0];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      <div className="absolute inset-0 bg-black/30" onClick={mode === "view" ? onClose : undefined} />

      <div className="relative z-10 flex h-[92%] w-full max-w-md flex-col overflow-hidden rounded-t-3xl bg-[#f2f2f5] dark:bg-[#1c1c1e]">
